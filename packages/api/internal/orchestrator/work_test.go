...
func gatedPauseFixture(t *testing.T) (pauseRemovalFixture, func(), <-chan error) {
	t.Helper()
	...
	node.SetSandboxClient(&pauseContextClient{pause: func(ctx context.Context, _ *orchestrator.SandboxPauseRequest) error {
		select {
		case <-gate:
			return nil
		case <-ctx.Done():
