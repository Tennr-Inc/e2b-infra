...
- **Sandbox ("client") nodes** run the orchestrator as a Nomad *system* job via `raw_exec`
  ...
  template caches. AWS sandbox nodes allocate huge pages on demand up to physical RAM less
  `client_reserved_host_memory_mib` (4 GiB by default), without a preallocated pool. Existing
  snapshots still require huge pages; ordinary free RAM alone does not indicate sandbox capacity.
  A systemd timer publishes `E2B/Capacity/SandboxMemoryUtilization` every minute, combining
  used plus reserved huge pages and physical memory pressure. Because memfd mappings do not
