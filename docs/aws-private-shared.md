...
Set `CLIENT_CLUSTER_SIZE=1`, `CLIENT_MAX_CLUSTER_SIZE=20`, and
`CLIENT_RESERVED_HOST_MEMORY_MIB=4096`. The maximum must be at least the minimum.
Equal bounds disable scaling and the capacity reporter. Build nodes retain their separate fixed
size and boot-time huge-page reservation settings.

Sandbox nodes use on-demand huge pages, with physical RAM minus the host reserve as the limit.
This removes the fixed preallocated/surplus split; it does not make memory unlimited. Huge pages
remain required by existing Firecracker snapshots. Dynamic allocation can still fail if the host
cannot obtain contiguous pages, so retain memory headroom rather than waiting for HTTP 500s.
...
Terraform updates launch templates without restarting existing nodes. Install the rendered
`install-capacity-reporter.sh.tftpl` on existing sandbox nodes after applying the IAM policy, and
verify `systemctl status e2b-capacity.timer` and `journalctl -u e2b-capacity.service`. To switch an
existing node's huge-page settings without restarting guests, set `nr_overcommit_hugepages` to
`(MemTotal in MiB - CLIENT_RESERVED_HOST_MEMORY_MIB) / 2` before setting `nr_hugepages=0`.
Existing allocations and reservations remain backed; free unreserved pages return to normal RAM.
Verify the CloudWatch metric and both scaling policies before relying on automatic scale-out.
