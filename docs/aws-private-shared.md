...
CLIENT_BASE_HUGEPAGES_PERCENTAGE=10
...
`CLIENT_RESERVED_HOST_MEMORY_MIB=4096`, and `CLIENT_BASE_HUGEPAGES_PERCENTAGE=10`
for the shared environment. The maximum node count must be at least the minimum.
Equal bounds disable scaling and the capacity reporter. Build nodes retain their separate fixed
size, 60% boot-time hugepage pool, and host-reserve settings. The generic Terraform base-pool
default remains 0%, so environments that omit the new setting keep dynamic allocation.

Sandbox nodes use 2 MiB huge pages with physical RAM minus the host reserve as the total budget.
At boot, 10% of that budget becomes a shared per-node pool; the remaining 90% can be allocated on
demand. For 64 GiB of reported RAM and a 4 GiB host reserve, this is 3,072 base pages (6 GiB)
plus a 27,648-page dynamic limit (54 GiB). Actual counts use the kernel's reported `MemTotal`
and round down to whole pages; base plus dynamic always equals the original eligible budget.
This leaves sandbox density, lazy snapshot loading, and overcommit behavior unchanged. It does
not reserve the full configured RAM of each sandbox. Preallocated pages cannot serve ordinary
page caches, so this trades some cache headroom for pages allocated before memory fragmentation.
...
The `e2b-capacity.timer` systemd timer emits `E2B/Capacity/SandboxMemoryUtilization` once per minute,
with the `AutoScalingGroupName` dimension. It measures the larger of huge-page reservation
utilization and physical memory utilization excluding unused, unreserved huge pages. It does not
publish until the orchestrator's gRPC listener is available. The node role may publish only to
this CloudWatch namespace. AWS target tracking adds instances above a 70% fleet average for this
metric or EC2 CPU, up to the maximum, with 600 seconds of warmup. Allow for metric collection,
instance boot, Nomad registration, and template loading; scaling cannot rescue a request that
already exhausted capacity. Keep extra minimum capacity for sudden bursts.
...
1. Launch one fresh node with the 10% setting. Before directing test work to it, confirm the
   ...
   An idle 6 GiB pool must not appear as 6 GiB of additional workload pressure in CloudWatch.
...
4. To roll back, restore `CLIENT_BASE_HUGEPAGES_PERCENTAGE=0` in the reviewed launch configuration
...
The 4 GiB setting is part of the Tot publisher's template definition, not a host-side override.
Publish and promote definition `tennr-coding-sandbox-v8` so new sessions use the smaller snapshot.
