...
# A shared per-node hugepage pool; the remaining eligible RAM stays dynamic.
CLIENT_RESERVED_HOST_MEMORY_MIB=4096
CLIENT_BASE_HUGEPAGES_PERCENTAGE=50
CLIENT_SANDBOX_MEMORY_LIMIT_MIB=32768
CLIENT_MAX_SANDBOXES_PER_NODE=8
ENVD_UPGRADE_TARGET=promoted
...
offer a disk option. The Tennr Tot source-only template requests `memoryMB: 4096`.
Its CLI and Corepack artifacts are prepared on the publisher; COPY staging uses the rootfs rather
than `/tmp`, and the preparation VM fetches source without installing repository dependencies.
...
New build records must show the requested 4 GiB RAM and effective 15 GiB free-rootfs allowance:
...
while `/tmp` remains separate. The current Tot publisher copies the CLI/Corepack toolchain and
fetches source inside its 4 GiB preparation VM; it no longer copies repository offline stores.

### Sandbox node capacity and scale-out

Set `CLIENT_CLUSTER_SIZE=1`, `CLIENT_MAX_CLUSTER_SIZE=20`,
`CLIENT_RESERVED_HOST_MEMORY_MIB=4096`, `CLIENT_BASE_HUGEPAGES_PERCENTAGE=50`,
`CLIENT_SANDBOX_MEMORY_LIMIT_MIB=32768`, and `CLIENT_MAX_SANDBOXES_PER_NODE=8`.
Keep `CLIENT_SERVER_MACHINE_TYPE=m8id.4xlarge` and the existing 4 GiB Tot guests.
Equal cluster bounds disable scaling and the capacity reporter. Build nodes retain their
separate fixed size, 60% boot-time hugepage pool, and host-reserve settings. Generic defaults
leave admission disabled for environments that do not configure both limits.

Sandbox nodes use 2 MiB huge pages with kernel-reported RAM minus the 4 GiB host reserve as
the total budget. At boot, 50% becomes a shared pool; the remainder may be allocated on demand.
On current workers the persistent pool is approximately 29 GiB. Eight fully used 4 GiB guests
therefore still require approximately 3 GiB of dynamic hugepages. Dynamic permission is not
physical backing, and fragmentation can still prevent allocation. Preallocated pages cannot
serve ordinary page caches. If this density remains unstable, lower the commitment cap to
24,576 MiB and six slots, and increase the persistent pool after checking host headroom.

The orchestrator reads `/etc/e2b/sandbox-capacity.json` at startup and commits a guest's full
configured RAM and one slot before creating its VM. Starts, auxiliary VMs, and teardown count;
only successful cleanup releases capacity. Fresh-VM checkpoints retain their commitment across
replacement. Workers refuse excess commitments with a structured capacity error so API placement
tries another worker. The 32 GiB limit is a worker commitment cap, not a change to guest RAM.

Startup reads back `vm.nr_hugepages` and `vm.nr_overcommit_hugepages` and refuses to register
Nomad if the allocation or total budget differs from the request. The check logs requested and
actual counts in `/var/log/user-data.log`. Since Nomad runs under Supervisor, a persisted
`supervisor.service` pre-start check repeats verification after `systemd-sysctl.service` on
subsequent boots; failures appear in `journalctl -u supervisor`. A successful sysctl write alone
does not prove the pool was fully allocated.

Worker mount definitions persist in `/etc/fstab` with `noauto`. A Nomad startup hook loads
`nbd` and mounts the binary buckets and hugetlbfs on every start, refusing registration if any
mount remains unavailable. Consul starts independently under Supervisor so its DNS can become
available while S3 mounts retry; making Supervisor itself depend on these mounts would create
a DNS startup cycle. This also restores the mounts after a drained-node reboot.

The motivating incident on September 18, 2026 reported `UFFDIO_COPY: cannot allocate memory`
while the host had no base pool. Later measurements showed substantial free RAM and no OOM
event, alongside cumulative hugepage allocation failures. Fragmentation is plausible, but those
observations do not establish the cause. The pool is a mitigation to validate, not a guarantee:
existing Firecracker snapshots still require huge pages, and dynamic allocation can still fail
when the host cannot obtain contiguous pages.

The `e2b-capacity.timer` systemd timer emits `E2B/Capacity/SandboxCapacityUtilization` once per
minute, with the `AutoScalingGroupName` dimension. It takes the maximum of committed guest RAM,
committed slots, persistent hugepage pressure with 2 GiB headroom, and physical memory pressure
excluding unused, unreserved hugepages. It reads the loopback-only `/capacity` endpoint on port
5008 and omits samples while the worker is not healthy. Unconfigured pools retain the legacy
`SandboxMemoryUtilization` metric. Six 4 GiB commitments on an eight-slot worker report at least
75%, even when their pages have not been touched.

AWS target tracking adds instances above a 70% fleet average for the capacity metric or EC2 CPU,
up to the maximum, with 600 seconds of warmup. Allow for metric collection, instance boot, Nomad
registration, and template loading; scaling cannot rescue a request that already exhausted
capacity. Keep extra minimum capacity for sudden bursts.

Both policies disable automatic scale-in. Newly launched nodes are protected from scale-in.
For manual removal, first prevent placement and drain running sandboxes, then verify all snapshot
uploads have succeeded in S3 before removing protection and lowering desired capacity. Stopping
an orchestrator or reducing capacity without this process can discard active work. The orchestrator
Nomad task now sends SIGTERM with a 24-hour kill timeout instead of the 5-second default. This
permits its 15-second propagation delay, sandbox drain, and upload cleanup; it is an upper bound,
not a mandatory wait. An empty node exits after cleanup. Pre-drain before stopping the task so
long-running sandboxes cannot consume the entire grace period and leave no time for uploads.
The existing upload retry budget is two hours; do not terminate a node merely because live sandbox
count reached zero. Check upload completion and errors too.

The API task has 240 seconds of shutdown grace: 15 seconds for health propagation, up to 75 seconds
for HTTP/gRPC drain, up to 95 seconds for detached pause work, and cleanup margin. Accepted native
pauses continue under an independent 80-second budget after client cancellation; their Redis
transition lease is 95 seconds and terminal DB writes have a separate 10-second budget. These
behaviors backport upstream commits `ace695a10` and `0a55a0ec` without migrating the Nomad deployment.
The ALB idle timeout is 90 seconds, above the 75-second API write deadline. This does not increase
the API's 70-second request deadline or remove the need for faster storage. Deferred rootfs export
remains unchanged. Validate real native pause/resume and forced client cancellation on the storage
canary before relying on the larger-workload path in production.

Terraform updates launch templates without restarting existing nodes. Roll out this change only
through an approved new-node canary; the code change itself does not update running nodes:

1. Launch one fresh node with the 50% setting and 32 GiB/eight-slot policy. Before directing test work to it, confirm the
   startup check passes and base plus dynamic counts match its RAM minus the host reserve.
   Verify `systemctl status supervisor e2b-capacity.timer` and the startup/capacity journals.
   Unused persistent pages must not appear as workload pressure in CloudWatch.
2. Repeatedly create, resume, and checkpoint representative sandboxes on that node while
   exercising template and page-cache churn. Compare failure rates and latency with the
   previous configuration. Capture `MemAvailable`, `HugePages_Total`, `HugePages_Free`,
   `HugePages_Rsvd`, `HugePages_Surp`, and deltas in `/proc/vmstat` hugepage allocation/failure
   counters. Cumulative counters alone do not identify the cause of a particular request failure.
3. Validate reboot startup on the drained canary and confirm the pool is checked before Nomad
   starts. Expand gradually only after allocation, memory pressure, and latency are acceptable.
   Verify both scaling policies and their CloudWatch metrics before relying on scale-out.
4. To roll back, restore the previous reviewed launch configuration, application artifacts, and scaling metric
   and replace affected nodes after draining their sandboxes. Do not retune hugepage sysctls,
   reboot, or replace nodes that still have active sandboxes as part of this rollout.

The 4 GiB setting is part of the Tot publisher's template definition and remains unchanged.
Existing sessions retain their original build and memory size.

Set `ENVD_UPGRADE_TARGET=promoted` after staging envd 0.7.1 in the pipeline bucket. The orchestrator
uses the existing version-gated live upgrade on creates as well as resumes, before exposing the
sandbox to callers. This removes request and response bodies from envd RPC debug logs without
requiring every template to be rebuilt. Older agents that lack live-upgrade support and failed
best-effort upgrades require separate verification; already-running guests do not upgrade until
their next start or resume. Verify the live guest version, not only the template's recorded version.

### September 23, 2026 capacity rollout

Production now uses launch-template version 7 on three `m8id.4xlarge` workers. Both previous
10% workers were made ineligible, drained, and retired only after sandbox and upload cleanup.
The API runs image digest `sha256:9db16eede018897a3446fff708d1287f3c029be5629c81cd064991e46ff3318a`;
the orchestrator build label is `capacity-20260923-d6d2f14e0a4b`. Envd 0.7.1 is promoted and the
live-upgrade target is enabled on starts and resumes.

Validation confirmed 14,789 persistent 2 MiB pages per worker (28.9 GiB), the 32,768 MiB/eight-slot
admission policy, healthy capacity endpoints and timers, and CloudWatch samples for the new metric.
On an empty standby canary, eight separate 4 GiB hugepage mappings were allocated and touched:
all 32 GiB succeeded, using 1,595 surplus pages, and all pages were released afterwards. This is a
host allocation check, not an eight-VM workload soak or proof against future fragmentation.
A fresh 4 GiB sandbox ran envd 0.7.1 and preserved a file and live process across two native
pause/resume cycles. Seven existing sandboxes were migrated with unchanged RAM; two others paused
naturally during the drain. No worker was terminated with live sandboxes or pending uploads.

The first cross-worker migration exposed a separate existing cache defect. Its snapshot upload
finished successfully at 19:52:34 UTC, but workers that looked up its rootfs before the upload
completed kept returning the cached missing-object error. `storageTemplate.Fetch` stores the
failure in `SetOnce`, and the template cache retains it for at least 25 hours. The sandbox was
recovered on a fresh worker. Subsequent migrations waited for the source worker's confirmed
snapshot-upload completion before requesting resume. The capacity and logging rollout does not
fix this cache defect; automatic cross-worker resumes still need retryable failed template loads.
Do not treat successful same-worker pause/resume as proof that this race is resolved.

### Local NVMe sandbox cache

The shared sandbox pool uses `CLIENT_SERVER_MACHINE_TYPE=m8id.4xlarge` and
`CLIENT_USE_INSTANCE_STORE=true`. The build pool is unchanged. Each sandbox node retains its
existing encrypted EBS root disk and mounts its 950 GB local NVMe disk at `/orchestrator` as XFS
with reflink enabled. Both sandbox COW files and exported snapshots are on that filesystem.
EC2 instance-store data is hardware-encrypted; it is ephemeral and not governed by the EBS KMS key.

The private setup bucket holds a checksum-named `instance-store.py` bootstrap. Compressed cloud-init
user data installs it before cache directories are created. The persistent Nomad startup hook
runs it again on reboot: existing managed XFS is reused, while a blank replacement instance-store
disk is initialized. Missing NVMe, foreign filesystems/partitions, conflicting mounts, or existing
files hidden beneath an absent cache mount block startup. EBS disks are never formatted.

This does not introduce new sandbox affinity: E2B already prefers the origin node's cache and
can restore uploaded snapshots from S3 on compatible nodes. Before replacing or stopping a node,
prevent placement, drain its sandboxes, and verify pending uploads completed successfully. EC2
stop/start loses NVMe contents; a normal reboot preserves them. Use a new-node canary to validate
mounts, same-node and cross-node resume, cancellation, and reboot before retiring old nodes.

Live validation on 2026-09-20 used the incident 4 GiB template and `pnpm install` in
`/sandbox/tennr`, producing 5.1 GiB of dependencies. Same-node pause took 3.2 seconds and resume
0.6 seconds. Disconnecting a separate pause request after one second still allowed the backend
to finish; the original sandbox resumed with its process, recent write, and dependency hashes
intact. A cross-node resume after verifying S3 upload hit the existing 40-second guest-readiness
deadline on its first cold attempt, then succeeded on the same destination in 11.3 seconds on
retry, preserving the same state. NVMe improves local export speed; cold reads from S3 can still
require a retry. Do not treat a failed resume request as proof that the snapshot is lost.
The guest-readiness ceiling is the existing `envd_timeout` Terraform variable (`ENVD_TIMEOUT`
in the orchestrator), separate from pause handling; this rollout leaves it at 40 seconds.
A normal reboot also preserved the NVMe marker file, remounted XFS with reflink, and restored
orchestrator health without manual repair.
Both retired workers completed sandbox and upload cleanup and exited without SIGKILL. Their
exit code was 1 because the sandbox-observer metrics flush timed out; this telemetry shutdown
error remains separate from the completed data drain.
