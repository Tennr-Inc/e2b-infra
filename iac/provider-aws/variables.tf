...
variable "client_max_cluster_size" {
  description = "Maximum sandbox nodes for memory- and CPU-based scale-out. Scale-in is disabled."
  type        = number
  default     = 20

  validation {
    condition     = var.client_max_cluster_size >= var.client_cluster_size && floor(var.client_max_cluster_size) == var.client_max_cluster_size
    error_message = "The maximum node count must be an integer at least client_cluster_size."
  }
}

variable "client_reserved_host_memory_mib" {
  description = "RAM kept available for the orchestrator and host services outside the base and dynamic huge-page budget."
  type        = number
  default     = 4096

  validation {
    condition     = var.client_reserved_host_memory_mib >= 4096 && floor(var.client_reserved_host_memory_mib) == var.client_reserved_host_memory_mib
    error_message = "Reserve at least 4096 MiB for host services."
  }
}

variable "client_base_hugepages_percentage" {
  description = "Percentage of sandbox-eligible RAM (after the host reserve) preallocated as a shared node hugepage pool."
  type        = number
  default     = 0

  validation {
    condition = (
      var.client_base_hugepages_percentage >= 0 &&
      var.client_base_hugepages_percentage <= 100 &&
      floor(var.client_base_hugepages_percentage) == var.client_base_hugepages_percentage
    )
    error_message = "The base hugepage percentage must be an integer from 0 through 100."
  }
}

variable "client_sandbox_memory_limit_mib" {
  description = "Full guest RAM commitment limit per worker; zero disables admission. A limit above the preallocated pool depends on dynamic hugepages."
  type        = number
  default     = 0

  validation {
    condition     = var.client_sandbox_memory_limit_mib >= 0 && floor(var.client_sandbox_memory_limit_mib) == var.client_sandbox_memory_limit_mib
    error_message = "The guest RAM limit must be a nonnegative integer in MiB."
  }
}

variable "client_max_sandboxes_per_node" {
  description = "Atomic per-worker slot limit, including starting and stopping VMs; zero disables admission. Set together with the guest RAM limit."
  type        = number
  default     = 0

  validation {
    condition     = var.client_max_sandboxes_per_node >= 0 && floor(var.client_max_sandboxes_per_node) == var.client_max_sandboxes_per_node
    error_message = "The sandbox slot limit must be a nonnegative integer."
  }
}
...
variable "envd_upgrade_target" {
  description = "Version-gated envd upgrade target for sandbox starts and resumes: off, promoted, or a staged revision."
  type        = string
  default     = "off"
}
