...
run "shared_private_topology" {
  ...
  assert {
    condition = (
      module.cluster.sandbox_capacity.min_nodes == 1 &&
      module.cluster.sandbox_capacity.max_nodes == 20 &&
      module.cluster.sandbox_capacity.memory_target == 70 &&
      module.cluster.sandbox_capacity.memory_scale_in_disabled &&
      module.cluster.sandbox_capacity.cpu_scale_in_disabled &&
      module.cluster.sandbox_capacity.scale_in_protection
    )
    error_message = "Sandbox nodes must scale out within 1–20 nodes without automatic scale-in."
  }

  assert {
    condition = (
      module.cluster.sandbox_capacity.base_hugepages_percentage == 0 &&
      module.cluster.sandbox_capacity.reserved_host_memory_mib == 4096 &&
      module.cluster.build_capacity.min_nodes == module.cluster.build_capacity.max_nodes &&
      module.cluster.build_capacity.base_hugepages_percentage == 60
    )
    error_message = "The generic sandbox default must remain dynamic while build nodes retain their separate settings."
  }

  assert {
    condition     = aws_lb.ingress.internal && aws_lb.ingress.load_balancer_type == "application" && aws_lb.ingress.idle_timeout == 90
    error_message = "The E2B application load balancer must remain internal with a 90-second idle timeout."
  }
...
run "shared_sandbox_hugepage_pool" {
  command = plan
  ...
  variables {
    ...
    client_server_machine_type       = "m8id.4xlarge"
  }

  assert {
    condition = (
      module.cluster.sandbox_capacity.base_hugepages_percentage == 50 &&
      module.cluster.sandbox_capacity.sandbox_memory_limit_mib == 32768 &&
      module.cluster.sandbox_capacity.max_sandboxes_per_node == 8 &&
      module.cluster.sandbox_capacity.capacity_metric == "SandboxCapacityUtilization" &&
      module.cluster.sandbox_capacity.instance_type == "m8id.4xlarge" &&
      module.cluster.sandbox_capacity.reserved_host_memory_mib == 4096 &&
      module.cluster.sandbox_capacity.min_nodes == 1 &&
      module.cluster.sandbox_capacity.max_nodes == 20 &&
      module.cluster.sandbox_capacity.memory_target == 70 &&
    ...
    error_message = "The shared 50% pool and eight-slot/32-GiB commitment policy must reach the existing worker type without changing build nodes."
  }
}

run "reject_negative_hugepage_percentage" {
  command = plan

  variables {
    client_base_hugepages_percentage = -1
  }

  expect_failures = [var.client_base_hugepages_percentage]
}

run "reject_excessive_hugepage_percentage" {
  command = plan

  variables {
    client_base_hugepages_percentage = 101
  }

  expect_failures = [var.client_base_hugepages_percentage]
}

run "reject_fractional_hugepage_percentage" {
  command = plan

  variables {
    client_base_hugepages_percentage = 1.5
  }
...
run "nvme_sandbox_cache" {
  command = plan
  variables {
    client_server_machine_type = "m8id.4xlarge"
    client_use_instance_store  = true
  }
  assert {
    condition     = module.cluster.sandbox_capacity.instance_type == "m8id.4xlarge" && module.cluster.sandbox_capacity.instance_store_cache && !module.cluster.build_capacity.instance_store_cache
    error_message = "The NVMe cache must be enabled only for the sandbox pool."
  }
}
