  ...
  mock_resource "aws_acm_certificate" {
    defaults = {
      ...
      domain_validation_options = [{
        ...
        resource_record_name  = "_validation.e2b.staging.internal.example.com"
...
run "private_staging_topology" {
  ...
  assert {
    condition     = contains(module.postgres.security_posture.ingress_security_groups, "sg-0fedcba9876543210")
    error_message = "PostgreSQL must allow explicitly configured private administration connectors."
  }
}
...
run "managed_certificate_in_public_route53" {
  command = plan

  variables {
    ingress_certificate_arn                = ""
    ingress_certificate_validation_zone_id = "Z0123456789ABCDEF"
  }

  assert {
    condition     = aws_route53_record.ingress_certificate_validation["*.e2b.staging.internal.example.com"].zone_id == "Z0123456789ABCDEF"
    error_message = "Terraform must publish ACM validation in the explicitly configured public hosted zone."
  }
}

run "security_group_only_ingress" {
  command = plan
  ...
  assert {
    condition     = length(aws_security_group.ingress.ingress) == 1
    error_message = "A private connector security group must be sufficient without allowing an entire CIDR."
  }
}
...
run "reject_missing_ingress_source" {
  command = plan
  ...
  expect_failures = [aws_security_group.ingress]
}
