...
# Keep guest commitments within the host budget. A limit above the persistent
# pool deliberately depends on dynamic hugepages, which can fail to allocate.
if (( ${SANDBOX_MEMORY_LIMIT_MIB} > 0 )); then
    if (( ${SANDBOX_MEMORY_LIMIT_MIB} + 2048 > (base_hugepages + overcommitment_hugepages) * 2 )); then
        echo "Sandbox RAM limit plus 2048 MiB headroom exceeds the host hugepage budget" >&2
        exit 1
    fi
    if (( ${SANDBOX_MEMORY_LIMIT_MIB} > base_hugepages * 2 )); then
        echo "WARNING: sandbox RAM limit exceeds the preallocated pool; full occupancy requires dynamic hugepages" >&2
    fi
    install -d -m 0755 /etc/e2b
    cat > /etc/e2b/sandbox-capacity.json <<EOF
{"memory_limit_mib": ${SANDBOX_MEMORY_LIMIT_MIB}, "sandbox_limit": ${MAX_SANDBOXES_PER_NODE}, "hugepage_headroom_mib": 2048}
EOF
fi
